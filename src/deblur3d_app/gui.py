...
def build_viewer() -> Viewer:
    ...
    @magicgui(
        ...
        # Controlled params (post-model in direct mode)
    ...
    def infer_w(
        ...
        """Main inference trigger — runs deblurring on the active Napari layer."""
        active = v.layers.selection.active
        ...
        # --- Resolve Hugging Face assets first (main thread → no freeze) ---
        try:
        ...
        # --- Launch worker (no GUI operations inside) ---
        ...
        def on_return(pred: np.ndarray):
            dt = time.time() - start
            # Include hyperparams in layer name
            layer_name = (
